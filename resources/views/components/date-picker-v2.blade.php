...
{{-- @dd("reApplyTimePicker_" . $attributes['in']) //reApplyTimePicker_0 --}}
{{-- @push('scripts') --}}
<script>
    window.addEventListener("reApplyTimePicker_{{ $attributes['in'] }}", event => {
        ...
        @if ($attributes['picker'] === 'date')
            // dd( '.flatpickr-' . $attributes['id']  ) // flatpickr-upgrades_proposals.0deadline
            ...
            // Luego, para activar manualmente el evento onReady
            el.trigger('onReady', el.selectedDates, el.formatDate(el.selectedDates[0]), el);
